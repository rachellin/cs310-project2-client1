...
def download(baseurl, display):
  ...
  try:
    ...
    print(binary_data)
    with open(body["assetname"], "wb") as file: 
      file.write(binary_data)
    # TODO: no such asset 
    if display:
      image = img.imread(body["assetname"])
      plt.imshow(image)
      plt.show()
...
def bucket(baseurl, api):
  try:
    ...
    for asset in assets:
      ...
      print(" ", asset.ETag)
      print(" ", asset.StorageClass)

    another = input("another page? [y/n]\n")
    if another == 'y':
      last_key = assets[-1].Key
      bucket(baseurl, "/bucket?startafter=" + last_key)
